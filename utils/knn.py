...
class KNNClassifier:
    def __init__(self):
        self.model = KNeighborsClassifier(n_neighbors=7, p=1, weights='distance')
    ...
    def train_and_evaluate(self, X_train, y_train, X_test, y_test, data_type, train_type='no_gan', dir_='results/knn'):
        model_filename = f'models/knn_without_cross_val_{data_type}_model.pkl'
        init_time = time.time()
        ...
        if train_type != 'no_gan':
            dir_ = 'results/knn_gan'
            model_filename = f'models/knn_gan_without_cross_val_{data_type}_model.pkl'
    ...
    def train_and_validate(self, X_train, y_train, data_type, train_type='no_gan',
                           save_directory='images/knn_cross_validation'):
        ...
        # save the trained model to a file
        model_filename = f'models/knn_with_cross_val_{data_type}_model.pkl'
        dump(self.model, model_filename)
        ...
        if train_type != 'no_gan':
            save_directory = 'images/knn_gan_cross_validation'
