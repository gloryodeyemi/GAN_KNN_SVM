...
def get_best_param(X_train, y_train, data_type):
    ...
    init_time = time.time()
    best_params = svm.grid_search(X_train, y_train, param_grid)
    cur_time = time.time()
...
def train_without_cross_val(X_train, y_train, X_test, y_test, data_type, cifar_type='CIFAR-10'):
    ...
    predictions = svm.train_and_evaluate(X_train, y_train, X_test, y_test, data_type)
...
def train_with_cross_val(X_train, y_train, data_type):
    ...
    svm.train_and_validate(X_train, y_train, data_type)
    print()
